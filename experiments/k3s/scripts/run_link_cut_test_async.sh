...
BASE=~/ba-self-healing/experiments/k3s/link-cut-tests/$SCENARIO/run-$RUN-router
mkdir -p "$BASE"
...
ROUTER_FAULT_LOG="/tmp/link-cut-${SCENARIO}-run-${RUN}.log"
S1_RECOVERY_LOG="$BASE/s1_interface_recovery.log"

ssh kim@$ROUTER_NAT "ip link show dev $ROUTER_IFACE" > "$BASE/interface_before_fault.txt"

ssh kim@$ROUTER_NAT "nohup bash -c '
  echo fault_start=\$(date -Is)
  sudo ip link set dev $ROUTER_IFACE down
  ip link show dev $ROUTER_IFACE
  sleep $DURATION
  sudo ip link set dev $ROUTER_IFACE up
...
ssh kim@$ROUTER_NAT "ip link show dev $ROUTER_IFACE" > "$BASE/interface_during_fault.txt" || true

echo "[RUN $RUN] Verbindungsabbruch läuft ${DURATION}s"
sleep "$DURATION"

echo "[RUN $RUN] Erzwinge zusätzliches Interface-Up von s1 über Router-NAT"
{
  echo "s1_recovery_start=$(date -Is)"
  timeout 30 ssh \
    -o ConnectTimeout=10 \
    -o ServerAliveInterval=5 \
    -o ServerAliveCountMax=2 \
    kim@$ROUTER_NAT "sudo ip link set dev $ROUTER_IFACE up"
  echo "s1_recovery_end=$(date -Is)"
} > "$S1_RECOVERY_LOG" 2>&1 || true

date -Is | tee "$BASE/recovery_time.txt"

sleep 5

ssh kim@$ROUTER_NAT "ip link show dev $ROUTER_IFACE" > "$BASE/interface_after_fault.txt"
