...
SCENARIO = Path("experiments/k3s/network-tests/latency-10min-async-limited")
...
for run_dir in sorted(SCENARIO.glob("run-*-router")):
    ...
    runs.append({
        ...
        "valid_tc": "delay 600s" in tc_during,
...
with open(OUT, "w") as f:
    f.write("# Latenztest 10min – asynchroner Monitor mit begrenzter Parallelität\n\n")

    f.write("## Ziel\n\n")
    f.write(
        "In diesem Test wurde das Verhalten der Testanwendung bei einer künstlich eingebrachten "
        "Netzwerklatenz von 10 Minuten untersucht. Im Unterschied zum synchronen Monitor wurde "
    ...
    f.write("| Erwartete Round-Trip-Zeit | ca. 1200s |\n")
    ...
    f.write(f"- `tc netem delay 600s` aktiv: {valid_tc}/{len(runs)}\n")
    ...
    f.write("\n## Interpretation\n\n")
    f.write(
        "Die Messreihe zeigt, dass die Testanwendung während der 10min-Latenz weiterhin teilweise erreichbar blieb, "
